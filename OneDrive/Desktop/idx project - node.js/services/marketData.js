...
async function refreshAllStockData() {
  try {
    const dataArray = await Promise.all(
      IDX_STOCKS.map((symbol) => fetchStock(symbol))
    );

    allStockData = IDX_STOCKS.map((symbol, idx) => ({
      symbol,
      ...dataArray[idx],
    }));
    ...
    console.error("Error fetching all stock data:", err);
  }
}

// Refresh every 60 seconds
// setInterval(refreshAllStockData, 60_000);
// refreshAllStockData();
